    ...
    def __init__(
        ...
        if upstream is None:
            ...
            if outputdir is None:
                self.setOutput(
                    'outputdir',
                    os.path.dirname(
                        os.path.abspath(self.getInput('covgzInput')[0])))
            else:
            ...
            if upstream.__class__.__name__ == 'compress_methyl':
                ...
                raise commonError(
                    'Parameter upstream must from compress_methyl.')
        ...
        self.setOutput('txtOutput', [
            os.path.join(self.getOutput('outputdir'),
                         self.getMaxFileNamePrefixV2(x)) + '-result.txt'
        ...
        if finishFlag:
            ...
            for i in range(multi_run_len):
                calcMethylV2(tbxInput=self.getInput('tbxInput')[i],
