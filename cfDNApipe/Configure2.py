...
class Configure2:
    ...
    def bismkrefcheck(cls, build):
        ...
        fafile = os.path.join(Configure2.getRefDir(), Configure2.getGenome() + '.fa')
        if not os.path.exists(fafile):
            print("Reference file " + fafile + " do not exist!")
            if build == True:
                url = "https://hgdownload.soe.ucsc.edu/goldenPath/" + Configure2.getGenome() + "/bigZips/" + Configure2.getGenome() + ".fa.gz"
                print("Download from URL:" + url + "......")
                urllib.request.urlretrieve(url, os.path.join(Configure2.getRefDir(), Configure2.getGenome() + '.fa.gz'))
                print("Uncompressing......")
                un_gz(os.path.join(Configure2.getRefDir(), Configure2.getGenome() + '.fa.gz'))
                print("Finished!")
            
        # check CpG island reference
        CpGislandfile = os.path.join(Configure2.getRefDir(), Configure2.getGenome() + 'cpgIslandExt.bed')
        if not os.path.exists(CpGislandfile):
            print("Reference file " + CpGislandfile + " do not exist!")
            if build == True:
                url = "http://hgdownload.soe.ucsc.edu/goldenPath/" + Configure2.getGenome() + "/database/cpgIslandExt.txt.gz"
                print("Download from URL:" + url + "......")
                urllib.request.urlretrieve(url, os.path.join(Configure2.getRefDir(), 'cpgIslandExt.txt.gz'))
                print("Uncompressing......")
                un_gz(os.path.join(Configure2.getRefDir(), 'cpgIslandExt.txt.gz'))
                regions = pd.read_csv(os.path.join(Configure2.getRefDir(), 'cpgIslandExt.txt'), sep = "\t", header = None)
                output_regions = regions.iloc[:, [1, 2, 3]]
                output_regions.to_csv(CpGislandfile, sep = "\t", header = False, index = False)
                print("Finished!")
        
        # check Bismark reference
        CTfiles = [os.path.join(Configure2.getRefDir(), 'Bisulfite_Genome/CT_conversion/' + x) for x in ['BS_CT.1.bt2', 'BS_CT.2.bt2', 'BS_CT.3.bt2', 'BS_CT.4.bt2', 'BS_CT.rev.1.bt2', 'BS_CT.rev.2.bt2', 'genome_mfa.CT_conversion.fa']]
        BAfiles = [os.path.join(Configure2.getRefDir(), 'Bisulfite_Genome/GA_conversion/' + x) for x in ['BS_GA.1.bt2', 'BS_GA.2.bt2', 'BS_GA.3.bt2', 'BS_GA.4.bt2', 'BS_GA.rev.1.bt2', 'BS_GA.rev.2.bt2', 'genome_mfa.GA_conversion.fa']]
    ...
    def bt2refcheck(cls, build):
        ...
        if not all(map(os.path.exists, bt2Ref)):
            ...
            if build == True:
                cmdline = "bowtie2-build -f --threads " + str(Configure2.getThreads()) + " " + fafile + " " + os.path.join(Configure2.getRefDir(), Configure2.getGenome())
