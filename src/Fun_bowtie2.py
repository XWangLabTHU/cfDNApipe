...
class bowtie2(StepBase):
    def __init__(self, 
        ...
        self.setOutput('unmapped', [x + '.unmapped.gz' for x in self.getParam('outPrefix')])
        ...
        for i in range(multi_run_len):
            tmp_cmd = self.cmdCreate(["bowtie2", 
                                       ...
                                       '--un-conc-gz', self.getOutput('unmapped')[i],
                                       '-p', self.getParam('threads'),
                                       '|',
                                       'samtools view -b -S -@', self.getParam('threads'), '-',
                                       '|',
                                       'samtools sort -@', self.getParam('threads'), '-o', self.getOutput('bamOutput')[i], '-'])
