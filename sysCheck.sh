...
if python -c 'import pkgutil; exit(not pkgutil.find_loader("seaborn"))'; then
	echo 'Package seaborn exist!'
	seaborn_flag=0
else
	echo 'Package seaborn not exist!'
	seaborn_flag=1
fi

if python -c 'import pkgutil; exit(not pkgutil.find_loader("sklearn"))'; then
	echo 'Package sklearn exist!'
	sklearn_flag=0
else
	echo 'Package sklearn not exist!'
	sklearn_flag=1
fi
...
if [ $seaborn_flag -ne 0 ]; then
	anaconda_list=$anaconda_list" seaborn"
fi
if [ $sklearn_flag -ne 0 ]; then
	anaconda_list=$anaconda_list" scikit-learn"
fi
